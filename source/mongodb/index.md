...
```
    ...
    getUser                                    Returns user information for a specified user. Run this method on the users database. The user must exist on the database on which the method runs.
    ...
    updateRole                                 Updates the roles profile on the database on which you run the method. An update to a field completely replaces the previous fields values.
    ...
    getRole                                    Returns role information for a specified role. Run this method on the roles database. The role must exist on the database on which the method runs.
